...
turn(State,NextState) :-
                ...
                write("Computer does move\n"),
	        bestNext(Col, State),
		%checkForWin(State,Columns,Columns,Col), 
...
% ----------------------MINIMAX--------------------------------%
% allNextStates schreibt alle möglichen nachfolge States in eine Liste.
...
% getMiniMax ruft auf jedes Element der Stateliste minMax auf.
% Also miniMax auf jeden State aus der Liste.
getMiniMax([], [], _).
...
% Computer ist dran und gewinnt
miniMax( 1000000, State, _) :- State = [Board|[CurrentPlayer]],
                			\+human(CurrentPlayer),
					 win_board(Board, CurrentPlayer).
% Computer ist dran und der Gegner kann gewinnen
miniMax( -1000000, State, _) :- State = [Board|[CurrentPlayer]],
                			\+human(CurrentPlayer),
					otherPlayer(CurrentPlayer,Player),
					 win_board(Board, Player).

% Human ist dran und gewinnt
miniMax(-1000000, State, _) :- State = [Board|[CurrentPlayer]],
                			 human(CurrentPlayer),
					 win_board(Board, CurrentPlayer).
% Human ist dran und Computer kann gewinnen.
miniMax( 1000000, State, _) :- State = [Board|[CurrentPlayer]],
                			human(CurrentPlayer),
					otherPlayer(CurrentPlayer,Player),
					 win_board(Board, Player).

% kein Spieler gewinnt und das Board ist voll.
...
% keiner gewinnt, das Board ist nicht voll und die Höhe = 0
miniMax(      Val,          States, 0) :- heuristik(Val, States).

% der Computer ist dran, keiner gewinnt, das Board ist nicht voll und die Höhe > 0
% dann minimax auf die nächste Tiefe anwenden 
% und mit getMax das Maximum der jetzigen Ebene berechnen.
...
% der Human ist dran, keiner gewinnt, das Board ist nicht voll und die Höhe > 0
% dann minimax auf die nächste Tiefe anwenden 
% und mit getMin das Minimum der jetzigen Ebene berechnen.
miniMax(     Val, State, H) :- State = [Board|[CurrentPlayer]],
					human(CurrentPlayer),
					otherPlayer(CurrentPlayer,Player),
					not(win_board(Board, Player)),
...
% alle Reihen in einer Liste
allRows(X,[]):- X= [].
allRows([H|Z],Board):- getHeads(Board,H), getTails(Board,T),allRows(Z,T).
...
% mehr oder weniger als anzal mal player symbol in der liste enthalten
% ist.
...
% falls das symbol des gegners enthalten ist und ersetzt diese durch eine
% leere liste.
...
% removeAllWithOX wendet emptyOrList auf die gesamte liste von listen an.
removeAllWithOX([],_,[]).
...
% gewinnReihe gibt alle Listen wieder die die Länge der amountToWin haben.
gewinnReihe(Rueck, X) :- amountToWin(Stones),
			 subList(Rueck, X),
			 length(Rueck, Stones).

% allGewinnReihenH erzeugt mit findall eine liste von listen von Gewinnreihen.
alleGewinnReihenH([],[]).
...
% gibt alle GewinnReihen der Spalten und Diagonalen wieder
allGewinnSpalten(Rueck, Board) :- allRows(R, Board),
				   allGewinnReihen(Rueck, R).

allGewinnDiagoLR(Rueck, Board) :- getRotDownRightDia(Board,R),
	                           allGewinnReihen(Rueck, R).

allGewinnDiagoRL(Rueck, Board) :- getDownRightDia(Board,R),
	                           allGewinnReihen(Rueck,R).
...
% allGewinnPosible erzeugt mitels findall eine liste von Gewinnmöglichkeiten.
allGewinnPosible(Rueck,Board) :- findall(Z, allGewinnPosH(Z, Board), R),
	                         leerRaus(R2,R),
				 conCat(Rueck, R2).


% getListOX gibt uns nun alle Gewinnreihen aus die nicht das Symbol des Gegners enthalten.
getListOX(Rueck, Player, Board) :- allGewinnPosible(R, Board),
	                           removeAllWithOX(R2, Player, R),
				   leerRaus(Rueck,R2).


% heuristikX erzeugt nun die punkte für den MaxPlayer(Computer) und heuristikO für
% den MinPlayer, somit ist der unterschied, das beim MinPlayer der Wert negativ ist.
...
% ruft heuristikX für den MaxPlayer und heuristikO für den MinPlayer, 
% wobei der Computer mit x der MaxPlayer ist.
heuristik(Wert, State) :- State = [Board|[CurrentPlayer]],
				   ...
				   Wert is W1 + W2.
% ruft heuristikX für den MaxPlayer und heuristikO für den MinPlayer, 
% wobei der Computer mit o der MaxPlayer ist.
heuristik(Wert, State) :- State = [Board|[CurrentPlayer]],
				   human(CurrentPlayer),
				   otherPlayer(CurrentPlayer,Player),
				   NewState = [Board|[Player]],
