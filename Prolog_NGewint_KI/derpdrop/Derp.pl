...
%Test3 x win win_board([[x,x,x,o,o],[x,o,x,o," "],[x,x,o,x," "],[o,o,x,o,x]],x).  %BUG
...
% - else go to the element left above the previous one and return the resulting list consisting of previous and following element
                % getSingleDia(X,Y,Board,Ret) :- widthBoard(B), Index is B, Index = X, getPos(X,Y,Board,R), Ret = [R], !.
                % getSingleDia(X,Y,Board,Ret) :- Nx is X + 1, Ny is Y - 1, getPos(X,Y,Board,W), getSingleDia(Nx,Ny,Board,R), Ret = [W|R], !. 
                %TODO
...
winInRDiags(Board,Player) :- getDownRightDia(Board,X), wonInColumns(X,Player), !. 
                ...
                %TODO for Rotated Board switch height and width
...
checkForWin(State,0,Columns,Ret) :- State = [Board|[Player]], otherPlayer(Player,Human), NewState = [Board|[Human]], stopWinning(NewState,Columns,Columns,Ret), !.
checkForWin(State,Col,Columns,Ret) :- (State = [Board|[Player]], columnPossible(Board,Col), placeInColumn(Board,Player,Col,NewBoard), win_board(NewBoard,Player)
                                        -> Ret = Col;
                                           Column is Col - 1, checkForWin(State,Column,Columns,Ret)), !.
...
stopWinning(State,0,Columns,Ret) :- State = [Board|[Player]], otherPlayer(Player,Computer), NewState = [Board|[Computer]], intelligentMove(NewState,Columns,Ret), !.
stopWinning(State,Col,Columns,Ret) :- (State = [Board|[Player]], columnPossible(Board,Col), placeInColumn(Board,Player,Col,NewBoard), win_board(NewBoard,Player)
                        -> Ret = Col;
                           Column is Col - 1, stopWinning(State,Column,Columns,Ret)), !.
...
intelligentMove(State,Columns,Ret) :-	
		...
		%State = [Board|[Player]], random_between(1,Columns,X), Ret=X.


		% minimax(Pos, BestNextState, Val)
		% Pos is a state, Val is its minimax value.
		% Best move from Pos leads to position BestNextPos.
		minimax(Pos, BestNextPos, Val) :-                     % Pos has successors
			widthBoard(X),	
			Pos = [Board|[_]],
			allPossibleColumns(Board,X,Ret,[]),    
			maybeTurn(Pos,Ret,NextPosList,[]),
		...
		min_to_move([_|[Player]]) :- human(X), Player == X.

		% max_to_move(State)
		% True if the next player to play is the MAX player.
		%max_to_move([_|[Player]]) :- \+human(X), Player == X.

		% utility asigned every NextState MinMax Values.
		% Previous player (MIN) has win.
		utility(State,Val) :- State = [Board,[Player]], win_board(Board,Player), human(X), Player == X, Val = -1.
		% Previous player (MAX) has win.
		%utility(State,Val) :- State = [Board,[Player]], win_board(Board,Player), \+human(X), Player == X, Val = 1.
		% No one win.
		utility(State,Val) :- State = [Board,[_]], draw_board(Board), Val = 0.

% return list of all columns with empty Slot
allPossibleColumns(Board,1,Ret,List) :- (columnPossible(Board,1) -> Ret = [1|List]; Ret = List), !.
allPossibleColumns(Board,X,Ret,List) :- (columnPossible(Board,X) -> List2 = [X|List], Xs is X - 1, allPossibleColumns(Board,Xs,Ret,List2); Xs is X - 1, allPossibleColumns(Board,Xs,Ret,List)), !.

% return a list of all NewStates
maybeTurn(_,[],ListOfNewStates,List) :- ListOfNewStates = List, !.
maybeTurn(State,ColList,ListOfNewStates,List) :- ColList = [H|T], move(State,H,NewState), NewList = [[NewState|[H]]|List], NewColList = T, maybeTurn(State,NewColList,ListOfNewStates,NewList), !.

% returns a list of Columns, where if you chose the Column 
...
choose(List, Elt) :-
		...
		% return a list of all NewStates
		maybeTurn1(_,[],ListOfNewStates,List) :- ListOfNewStates = List, !.
		maybeTurn1(State,ColList,ListOfNewStates,List) :- ColList = [H|T], move(State,H,NewState), NewList = [NewState|List], NewColList = T, maybeTurn(State,NewColList,ListOfNewStates,NewList), !.
		...
		%turnMove(State,NextState) :-
		%			State = [Board|[CurrentPlayer]],
		%               	widthBoard(Columns),
		%			Z = allPossibleColumns(Board,Columns,Ret,[]),
		%                	random_member(X,Z),                
		%			move(State,X,NextState), !.
		%
		%trun_between(1,Columns,Board,Ret) :- (random_between(1,Columns,X), columnYPossible(Board,X)
		%					-> Ret = X;
		%					turn_between(1,Columns,Board,Ret)).
		%heightBoard(3).
