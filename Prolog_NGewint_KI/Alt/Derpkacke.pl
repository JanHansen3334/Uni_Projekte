...
stopWinning(State,0,Columns,Ret) :- State = [Board|[Player]], otherPlayer(Player,Computer), NewState = [Board|[Computer]], intelligentMove(NewState,Columns,Columns,Ret), !.
...
intelligentMove(State,Col,Columns,Ret) :- ((checkNextTurnWin(State,Col,Columns,Return), Return == z) 
					-> random_member(1,Columns,Column), Ret = Column;
                                       write(Col), write(Colums), helpFunction(State,Col,Columns,Return1), write(Return1), Ret = Return1).


helpFunction(State,Col,Columns,Return1) :- (random_between(1,Columns,Y), X = Y, State = [Board|[Player]], columnPossible(Board,X), placeInColumn(Board,Player,X,NewBoard), NewBoard1 = NewBoard, columnPossible(NewBoard1,X), otherPlayer(Player,PlayerX), placeInColumn(NewBoard1,PlayerX,X,NewNewBoard), NewNewBoard1 = NewNewBoard, win_board(NewNewBoard1,PlayerX) -> helpFunction(State,Col,Columns,Return1);
				Return1 = X), !.

% check if the player anyways win after doing our turn
checkNextTurnWin(State,0,Columns,Ret) :- Ret = z.
checkNextTurnWin(State,Col,Columns,Ret) :- (State = [Board|[Player]], columnPossible(Board,Col), placeInColumn(Board,Player,Col,NewBoard), NewBoard1 = NewBoard, columnPossible(NewBoard1,Col), otherPlayer(Player,PlayerX), placeInColumn(NewBoard1,PlayerX,Col,NewNewBoard), NewNewBoard1 = NewNewBoard, win_board(NewNewBoard1,PlayerX) 
